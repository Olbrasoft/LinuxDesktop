...
public interface IPointerService : IAsyncDisposable
{
    ...
    Task<(int X, int Y)?> GetPointerPositionAsync(CancellationToken ct = default);
    ...
    Task<(int X, int Y, int Width, int Height)?> GetActiveWindowGeometryAsync(CancellationToken ct = default);
}
